...
const stub = {
  records: [] as BookingMetric[],
  bookings: [] as MechanicBookingRow[],
  ratings: emptyRatingSnapshot(),
};
const captures = {
...
import {
  getMechanicIncomeMetrics,
  getMechanicStatsMetrics,
  getPublicMechanicMetrics,
} from "@/lib/mechanic/mechanic-metrics.service";
...
describe("mechanic snapshot services", () => {
  test("validates income paging and states before reading metric sources", async () => {
    for (const params of [
      ...
      expect(await getMechanicIncomeMetrics(MECHANIC_ID, params)).toMatchObject(
        { ok: false, status: 400 },
      );
    }
  ...
  test("income preserves full summaries while returning a bounded second page", async () => {
    ...
    const result = await getMechanicIncomeMetrics(MECHANIC_ID, {
      page: 2,
      limit: 8,
      state: "pending",
      now: NOW,
  ...
  test("review pagination leaves full rating totals unchanged", async () => {
    const reviews = Array.from({ length: 12 }, (_, index) =>
      makeReviewRow({ review_id: `r-${index}` }),
    );
    stub.ratings = {
      average: 5,
      count: 12,
      distribution: [{ stars: 5, count: 12 }],
      reviews,
    };
    const result = await getMechanicStatsMetrics(MECHANIC_ID, {
      reviewPage: 3,
      now: NOW,
    });
    expect(result.ok).toBe(true);
    ...
    expect(result.data.reviews.map((review) => review.id)).toEqual([
      "r-10",
      "r-11",
    ]);
    expect(result.data.reviewPagination).toEqual({
      page: 3,
      pageSize: 5,
      total: 12,
      totalPages: 3,
    ...
    expect(
      await getMechanicStatsMetrics(MECHANIC_ID, { reviewPage: -1 }),
  ...
  test("public ratings and completions come from captured rows, not mutable profile counters", async () => {
    ...
    expect(await getPublicMechanicMetrics(MECHANIC_ID)).toEqual({
      ratingAvg: 4.3,
      ratingCount: 60,
      completedJobs: 1,
    });
    expect(captures.captureMechanicBookingRows.mock.calls).toEqual([
      [MECHANIC_ID],
    ]);
  });
});
...
describe("complete mechanic schedule", () => {
  test("keeps older open work beyond sixty recent completions and counts before filtering", async () => {
    stub.bookings = Array.from({ length: 61 }, (_, index) =>
      makeBookingRow({
        booking_id: `done-${index}`,
        status: "completed",
        scheduled_at: new Date("2026-09-16T08:00:00Z"),
      }),
    );
    stub.bookings.push(
      makeBookingRow({
        booking_id: "older-open",
        status: "en_route",
        scheduled_at: new Date("2026-09-15T08:00:00Z"),
      }),
    );
    const result = await getMechanicSchedule(MECHANIC_ID, {
      status: "en_route",
      now: NOW,
    });
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.data.bookings.map((booking) => booking.id)).toEqual([
      "older-open",
    ]);
  ...
  test("clamps out-of-range pages and loads item snapshots only for displayed rows", async () => {
    stub.bookings = Array.from({ length: 10 }, (_, index) =>
      makeBookingRow({ booking_id: `id-${index}` }),
    );
    const result = await getMechanicSchedule(MECHANIC_ID, {
      page: 999,
      limit: 8,
  ...
  test("rejects unknown filter or invalid page before storage", async () => {
    expect(
      await getMechanicSchedule(MECHANIC_ID, { status: "unknown" }),
    ).toMatchObject({ ok: false, status: 400 });
    expect(await getMechanicSchedule(MECHANIC_ID, { page: -1 })).toMatchObject({
      ok: false,
      status: 400,
    });
