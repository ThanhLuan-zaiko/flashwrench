...
export const metricStubs = {
  ...
  history: new Map<
    string,
    MetricPage<{ status: string | null; at: Date | null }>
  >(),
...
export const metricRepoMocks = {
  metricPaymentRefs: mock(
    async (id: string, state: string | null) =>
      metricStubs.refs.get(metricStubKey(id, state)) ?? {
        rows: [],
        pageState: null,
      },
  ),
  metricReceiptById: mock(
    async (id: string) => metricStubs.receipts.get(id) ?? null,
  ),
  metricHistoryPage: mock(
    async (id: string, state: string | null) =>
      metricStubs.history.get(metricStubKey(id, state)) ?? {
        rows: [],
        pageState: null,
      },
  ),
  metricReviewPage: mock(
    async (id: string, state: string | null) =>
      metricStubs.reviews.get(metricStubKey(id, state)) ?? {
        rows: [],
        pageState: null,
      },
  ),
