...
export function operationsSnapshot(
  ...
  const scope: OperationsSnapshot["scope"] = {
    ...metricCoverage(now), kind: "scheduled-month", month,
  };
  const totals: OperationsSnapshot["totals"] = {
    bookings: 0, open: 0, unassigned: 0, completed: 0, cancelled: 0,
    noShow: 0, bookingValue: 0, collected: 0, outstanding: 0, completionRate: 0,
  };
  const statuses = MECHANIC_BOOKING_STATUSES.map((status) => ({ status, count: 0 }));
  const daily = Array.from({ length: days }, (_, index) => ({
    day: `${month}-${String(index + 1).padStart(2, "0")}`,
    bookings: 0, completed: 0, value: 0,
  }));
  const byDay = new Map(daily.map((point) => [point.day, point]));
  const seen = new Set<string>();
  for (const { booking, status, payments, completedAt } of records) {
    if (seen.has(booking.booking_id) || booking.month_bucket !== month) continue;
    ...
    if (status === "completed") {
      totals.completed += 1;
      if (!validMetricDate(completedAt) || completedAt > now) scope.undatedCompletions += 1;
    }
    ...
    for (const receipt of payments) {
      if (seenPayments.has(receipt.payment_id) || receipt.status !== "paid") continue;
      seenPayments.add(receipt.payment_id);
      paid = addMetricMoney(paid, metricMoney(receipt.amount));
      if (!validMetricDate(receipt.paid_at) || receipt.paid_at > now) scope.undatedPayments += 1;
    }
    totals.collected = addMetricMoney(totals.collected, paid);
    if (status === "completed" && booking.payment_status !== "refunded") {
      totals.outstanding = addMetricMoney(totals.outstanding, Math.max(0, total - paid));
    }
    if (validMetricDate(booking.scheduled_at)) {
      const point = byDay.get(dayKey(booking.scheduled_at, booking.timezone ?? scope.timeZone));
      if (point) {
        point.bookings += 1;
        if (status === "completed") point.completed += 1;
        if (status !== "cancelled" && status !== "no_show") point.value = addMetricMoney(point.value, total);
      } else scope.unplacedBookings += 1;
    } else scope.unplacedBookings += 1;
  }
  const settled = totals.completed + totals.cancelled + totals.noShow;
  totals.completionRate = settled ? Math.round(totals.completed / settled * 100) : 0;
