...
export function mechanicStatsSnapshot(
  ...
  if (!validMetricPaging(reviewPage, pageSize)) throw new Error("Invalid review pagination.");
  const scope: MechanicStatsSnapshot["scope"] = { ...metricCoverage(now), kind: "assigned-history" };
  const stats: MechanicStats = {
    openJobs: 0, completedJobs: 0, cancelledJobs: 0, noShowJobs: 0,
    completedThisMonth: 0, revenueTotal: 0, revenueThisMonth: 0,
    ratingAvg: ratings.average, ratingCount: ratings.count, completionRate: 0,
  };
  const monthly = recentMonthKeys(now, 6).map((month) => ({ month, completed: 0, revenue: 0 }));
  ...
  for (const { booking, status, payments, completedAt } of records) {
    ...
    for (const receipt of payments) {
      if (paymentIds.has(receipt.payment_id) || receipt.status !== "paid") continue;
      ...
      if (isSameMonth(receipt.paid_at, now)) stats.revenueThisMonth = addMetricMoney(stats.revenueThisMonth, amount);
    }
  }
  const settled = stats.completedJobs + stats.cancelledJobs + stats.noShowJobs;
  stats.completionRate = settled ? Math.round(stats.completedJobs / settled * 100) : 0;
  const reviewPagination = metricPagination(ratings.reviews.length, reviewPage, pageSize);
  const offset = (reviewPagination.page - 1) * pageSize;
  const reviews = ratings.reviews.slice(offset, offset + pageSize).map((row) => ({
    id: row.review_id, customerName: row.customer_name ?? "", rating: row.rating ?? 0,
    comment: [row.title, row.body].filter(Boolean).join(" — "),
    createdAt: validMetricDate(row.created_at) ? row.created_at.toISOString() : null,
  }));
  return { stats, monthly, ratings: ratings.distribution, reviews, reviewPagination, scope };
}
