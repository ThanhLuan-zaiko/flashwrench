...
export function mechanicIncomeSnapshot(
  ...
  options: {
    page?: number;
    pageSize?: number;
    state?: MechanicIncomeState | "all";
  } = {},
): MechanicIncomeSnapshot {
  const page = options.page ?? 1;
  const pageSize = options.pageSize ?? 8;
  if (!validMetricPaging(page, pageSize))
    throw new Error("Invalid income pagination.");
  const state = options.state ?? "all";
  if (!["all", "paid", "pending", "refunded"].includes(state))
    throw new Error("Invalid income state.");
  const scope: MechanicIncomeSnapshot["scope"] = {
    ...metricCoverage(now),
  ...
  const counts: MechanicIncomeSnapshot["counts"] = {
    all: 0,
    paid: 0,
    pending: 0,
    refunded: 0,
  ...
  for (const { booking, payments, status, completedAt } of records) {
    ...
    for (const payment of payments) {
      ...
      if (isSameDay(payment.paid_at, now))
        summary.today = addMetricMoney(summary.today, amount);
      if (isSameWeek(payment.paid_at, now))
        summary.week = addMetricMoney(summary.week, amount);
      if (isSameMonth(payment.paid_at, now))
        summary.month = addMetricMoney(summary.month, amount);
    }
    if (
      status === "completed" &&
      (!validMetricDate(completedAt) || completedAt > now)
    ) {
      scope.undatedCompletions += 1;
    }
    const outstanding =
      status === "completed" && booking.payment_status !== "refunded"
    ...
    if (status === "completed" && outstanding > 0) entryState = "pending";
      ...
      received > 0 ||
      (status === "completed" && booking.payment_status === "paid")
    )
      entryState = "paid";
    else if (refunded || booking.payment_status === "refunded")
      entryState = "refunded";
