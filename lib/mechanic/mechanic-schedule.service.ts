...
import type { MechanicBookingStatus, MechanicBookingSummary } from "./mechanic.types";
import { dayKey } from "./mechanic-period";
import { isOpenBookingStatus, MECHANIC_BOOKING_STATUSES, parseBookingStatus } from "./mechanic-status";
import { type MetricPagination, metricPagination, validMetricPaging } from "./mechanic-metrics.types";
...
export async function getMechanicSchedule(
  ...
  if ((status !== "all" && !parseBookingStatus(status)) || !validMetricPaging(page, size)) {
    return { ok: false, status: 400, errors: { form: "Bộ lọc hoặc trang lịch hẹn không hợp lệ." } };
  }
  const now = params.now ?? new Date();
  const rows = (await captureMechanicBookingRows(mechanicId))
    .filter((row) => parseBookingStatus(row.status))
    .sort((left, right) => (right.scheduled_at?.getTime() ?? 0) - (left.scheduled_at?.getTime() ?? 0) || left.booking_id.localeCompare(right.booking_id));
  const counts = Object.fromEntries(["all", ...MECHANIC_BOOKING_STATUSES].map((key) => [key, 0])) as MechanicSchedulePayload["counts"];
  ...
  for (const row of rows) {
    ...
    if (row.scheduled_at && dayKey(row.scheduled_at, row.timezone ?? undefined) === dayKey(now, row.timezone ?? undefined)) overview.today += 1;
  }
  const selected = status === "all" ? rows : rows.filter((row) => row.status === status);
  const pagination = metricPagination(selected.length, page, size);
  const offset = (pagination.page - 1) * pagination.pageSize;
  return {
    ok: true,
    data: {
      bookings: await mapBookingSummaries(selected.slice(offset, offset + size)),
      counts, pagination, overview,
    },
  };
}
