...
import { DISPATCH_SECTIONS, type DispatchSectionId } from "./dispatch-sections";

type DispatchSidebarProps = {
  active: DispatchSectionId;
  collapsed: boolean;
  mobileOpen: boolean;
...
// Sidebar owns navigation only. Visual language matches the mechanic and
// admin sidebars: rounded links, zinc borders, monochrome active state.
  ...
  return (
    <>
      {mobileOpen && (
        <button
          type="button"
          aria-label="Đóng menu điều phối"
              ...
              return (
                <li key={section.id}>
                  <Link
                    ...
                    className={`flex min-h-[44px] w-full items-center gap-3 rounded-xl border border-zinc-200 px-3 py-2 text-sm font-medium transition-colors duration-200 hover:bg-zinc-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-zinc-500 motion-safe:active:scale-[0.99] dark:border-zinc-800 dark:hover:bg-zinc-900 ${
                      selected
          ...
          {!collapsed && (
            <div className="mt-4 rounded-2xl border border-zinc-200 p-3 dark:border-zinc-800">
              <p className="flex items-center gap-1.5 text-xs font-semibold text-zinc-800 dark:text-zinc-200">
                ...
                />
                Trực tuyến điều phối
              </p>
              <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
                {displayName
                  ? `${displayName} đang trực. Đơn mới hiện ngay trên bàn.`
                  : "Đơn mới sẽ hiện ngay trên bàn điều phối."}
              </p>
            </div>
          )}
        </nav>
        ...
        <div className="flex flex-col gap-1.5 border-t border-zinc-200 p-3 dark:border-zinc-800">
          ...
          {!collapsed && (
            <button
              type="button"
              onClick={onLogout}
              className="flex min-h-[44px] w-full items-center justify-center gap-2 rounded-xl border border-zinc-200 px-3 py-2 text-sm font-medium text-zinc-700 transition-colors duration-200 hover:bg-zinc-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-zinc-500 motion-safe:active:scale-[0.99] dark:border-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-900"
            >
              Đăng xuất
            </button>
          )}
