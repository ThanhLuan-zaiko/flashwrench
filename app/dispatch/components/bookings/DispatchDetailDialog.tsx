...
import {
  useDispatchBooking,
  useDispatchBookingRealtime,
} from "@/hooks/dispatch";
import { DispatchActions } from "./DispatchActions";
import { DispatchBookingInfo } from "./DispatchBookingInfo";

type DispatchDetailDialogProps = {
  bookingId: string;
  onClose: () => void;
  onToast: (
...
// Dialog owns one booking: customer, vehicle, line items, timeline and the
// dispatcher workflow (confirm, assign, cancel). Live booking events
// refresh it so another dispatcher's edit never goes stale under our feet.
export function DispatchDetailDialog({
  bookingId,
  onClose,
  onToast,
}: DispatchDetailDialogProps) {
  const query = useDispatchBooking(bookingId);
  useDispatchBookingRealtime(bookingId);
      ...
      >
        ...
        {booking && (
          <div className="mt-3 flex flex-col gap-4">
            <DispatchBookingInfo booking={booking} />
            <DispatchActions
              booking={booking}
              onClose={onClose}
              onToast={onToast}
