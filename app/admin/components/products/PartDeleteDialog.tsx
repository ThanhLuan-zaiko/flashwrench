...
// Shared confirm dialog for the parts catalog. Soft delete moves to trash
// (restorable); hard delete is permanent and requires typing the slug to
// unlock the button.
export function PartDeleteDialog({
  ...
  const canConfirm =
    !pending && (!isHard || confirmText.trim().toLowerCase() === target.slug);
  const kindLabel = target.kind === "category" ? "danh mục" : "sản phẩm";
      ...
      <div className="relative w-full max-w-md rounded-2xl border border-zinc-200 bg-white p-5 shadow-xl dark:border-zinc-800 dark:bg-zinc-950">
        <div className="flex items-start justify-between gap-3">
          <p className="flex items-center gap-2 text-sm font-bold text-zinc-900 dark:text-zinc-50">
            ...
            {isHard ? `Xóa vĩnh viễn ${kindLabel}` : `Xóa mềm ${kindLabel}`}
          </p>
          <button
            type="button"
            onClick={onClose}
        ...
        {isHard ? (
          <div className="mt-3 rounded-xl border border-zinc-200 p-3 dark:border-zinc-800">
            <p className="text-xs font-semibold text-zinc-800 dark:text-zinc-200">
              Hành động không thể hoàn tác. Mọi dữ liệu của {kindLabel} này sẽ
              mất vĩnh viễn.
            </p>
            <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
              Nhập đúng mã{" "}
              <span className="font-mono font-semibold">{target.slug}</span> để
              xác nhận.
            </p>
            <input
              value={confirmText}
              ...
              aria-label="Nhập mã để xác nhận xóa vĩnh viễn"
          ...
          <p className="mt-3 rounded-xl bg-zinc-100 px-3 py-2.5 text-xs text-zinc-600 dark:bg-zinc-900 dark:text-zinc-300">
            Mục này sẽ chuyển vào Thùng rác và ẩn khỏi khách hàng. Bạn có thể
