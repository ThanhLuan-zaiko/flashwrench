...
// Mechanic step: nearest verified, online mechanics first (the query
// re-sorts when the map pin moves). Auto-dispatch stays the default so
// an empty directory never blocks a booking.
export function MechanicSection({
  ...
  return (
    <fieldset disabled={disabled} className="flex flex-col gap-2">
      ...
      <label
        className={`flex min-h-[44px] cursor-pointer items-center gap-3 rounded-xl border px-3 py-2.5 transition-colors duration-200 ${
          value === null
      ...
      {mechanics.isSuccess && items.length === 0 && (
        <p className="text-[11px] text-zinc-500 dark:text-zinc-400">
          Hiện chưa có thợ trực tuyến. Cứ giữ “tự điều thợ”, đơn của bạn sẽ được
          điều phối ngay khi có thợ rảnh.
        </p>
      )}
